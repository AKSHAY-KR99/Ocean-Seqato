...
def calculate_total_sales_amount(fund_invoice_object, contract_object):
    ...
    if contract_object.fixed_fee_type == models.TERMS_TYPE_AMOUNT:
        if fund_invoice_object.contract_category is not None and fund_invoice_object.fixed_fee_value != contract_object.fixed_fee_value and fund_invoice_object.fixed_fee_value is not None:
...
def payment_acknowledgment_mail_sme(subject, recipient_name, paying_amount, supplier_name, recipient_email):
    """
    ...
    message = render_to_string('transaction_app/payment_acknowledgment_sme.html', {
        ...
        'admin_name': settings.SENDER_NAME,
        'logo_path': settings.BACKEND_URL[:-1] + settings.MEDIA_URL + 'logo/'
...
def get_codat_xero_balance_sheet_data(company_id, user_object):
    ...
    balance_sheet_response = requests.get(
        f"https://api.codat.io/companies/{company_id}/data/financials/balanceSheet?periodLength=1&periodsToCompare=12",
        headers={"Content-Type": "application/json",
                 "Accept": "application/json",
                 "Authorization": settings.CODAT_AUTHORIZATION_KEY})
    ...
    return codat_balance_sheet_response(balance_sheet_response.json(), user_object, info)


def get_codat_xero_profit_and_loss_data(company_id, user_object):
    profit_loss_response = requests.get(
        f"https://api.codat.io/companies/{company_id}/data/financials/profitAndLoss?periodLength=1&periodsToCompare=12",
        headers={"Content-Type": "application/json",
                 "Accept": "application/json",
                 "Authorization": settings.CODAT_AUTHORIZATION_KEY})
    comapny_obj = get_codat_xero_company_info(company_id)
...
def codat_profit_loss_response(json_response, user_object, info):
    ...
    accouts_objects = {'heads': [], 'items': [], 'data':{}}
    accouts_objects['data']['blank'] = {}
    accouts_objects['data']['blank']['name'] = ''
    accouts_objects['data']['blank']['style'] = row_format
    accouts_objects['data']['blank']['values'] = {}
    for accounts in json_response['reports']:
        for main_type in accounts:
            if accounts['fromDate'] not in accouts_objects['heads']:
                accouts_objects['heads'].append(accounts['fromDate'])
            if main_type not in ['fromDate', 'toDate']:
                if main_type not in accouts_objects['items']:
                    accouts_objects['items'].append('blank')
                    accouts_objects['items'].append(main_type)
                if type(accounts[main_type])==dict:
                    if main_type not in accouts_objects['data']:
                        accouts_objects['data'][main_type] = {}
                        accouts_objects['data'][main_type]['name'] = accounts[main_type]['name']
                        accouts_objects['data'][main_type]['style'] = head1_format
                        accouts_objects['data'][main_type]['values'] = {}
                    if f"Total {accounts[main_type]['name']}" not in accouts_objects['items']:
                        accouts_objects['items'].insert(accouts_objects['items'].index(main_type)+1, f"Total {accounts[main_type]['name']}")
                    if f"Total {accounts[main_type]['name']}" not in accouts_objects['data']:
                        accouts_objects['data'][f"Total {accounts[main_type]['name']}"] = {}
                        accouts_objects['data'][f"Total {accounts[main_type]['name']}"]['name'] = f"Total {accounts[main_type]['name']}"
                        accouts_objects['data'][f"Total {accounts[main_type]['name']}"]['style'] = copy.deepcopy(row_format)
                        accouts_objects['data'][f"Total {accounts[main_type]['name']}"]['style']['bold'] = True
                        accouts_objects['data'][f"Total {accounts[main_type]['name']}"]['values'] = {}
                    accouts_objects['data'][f"Total {accounts[main_type]['name']}"]['values'][accounts['fromDate']] = accounts[main_type]['value']
                    for value1 in accounts[main_type]['items']:
                        if value1['name'] not in accouts_objects['items']:
                            insert_index = accouts_objects['items'].index(f"Total {accounts[main_type]['name']}")
                            accouts_objects['items'].insert(insert_index, value1['name'])
                        if value1['name'] not in accouts_objects['data']:
                            accouts_objects['data'][value1['name']] = {}
                            accouts_objects['data'][value1['name']]['name'] = value1['name']
                            accouts_objects['data'][value1['name']]['style'] = row_format
                            accouts_objects['data'][value1['name']]['values'] = {}
                            accouts_objects['data'][value1['name']]['values'][accounts['fromDate']] = value1['value']
                        else:
                            accouts_objects['data'][value1['name']]['values'][accounts['fromDate']] = value1['value']
                elif type(accounts[main_type])==float:
                    if main_type not in accouts_objects['data']:
                        accouts_objects['data'][main_type] = {}
                        accouts_objects['data'][main_type]['name'] = re.sub(r"(\w)([A-Z])", r"\1 \2", main_type).title()
                        accouts_objects['data'][main_type]['style'] = copy.deepcopy(head1_format)
                        accouts_objects['data'][main_type]['style']['bg_color'] = '#e4ecf6'
                        accouts_objects['data'][main_type]['values'] = {}
                    accouts_objects['data'][main_type]['values'][accounts['fromDate']] = accounts[main_type]

    df = pd.DataFrame()
    writer = pd.ExcelWriter(file_path + f"{info['type']}.xlsx", engine='xlsxwriter')
    df.to_excel(writer, sheet_name=info['sub_title'])
    # Get the xlsxwriter workbook and worksheet objects.
    workbook = writer.book
    worksheet = writer.sheets[info['sub_title']]
    title_head = {'font_size': 14,
                    'font_color': '#6f749b',
                    'bg_color': '#ffffff',
                    'bold': True,
                    'align': 'center',
                    'valign': 'vcenter',
                    'bottom': 0}
    title_company = copy.deepcopy(title_head)
    title_company['font_size'] = 11
    title_date = copy.deepcopy(title_head)
    title_date['font_size'] = 11
    date_title = f"{datetime.strptime(accouts_objects['heads'][-1], '%Y-%m-%dT%H:%M:%S').strftime('%b %Y')} - {datetime.strptime(accouts_objects['heads'][0], '%Y-%m-%dT%H:%M:%S').strftime('%b %Y')}"
    worksheet.merge_range(0, 0, 0, len(accouts_objects['heads']), info['sub_title'], workbook.add_format(title_head))
    worksheet.merge_range(1, 0, 1, len(accouts_objects['heads']), info['title'].upper(), workbook.add_format(title_company))
    worksheet.merge_range(2, 0, 2, len(accouts_objects['heads']), date_title, workbook.add_format(title_date))
    worksheet.merge_range(3, 0, 3, len(accouts_objects['heads']), '', workbook.add_format(title_date))
    worksheet.set_row(0, 20)
    worksheet.set_row(1, 20)
    worksheet.set_row(2, 20)

    highlight_row_heads = ['NET PROFIT']
    total_return_row_heads = ['SALES']
    total_return_value = 0
    row, column = 4, 0
    column1_width = 0
    worksheet.set_row(row, 30)
    worksheet.write(row, column, '', workbook.add_format(date_format))
    for heading in accouts_objects['heads']:
        column += 1
        date_style = copy.deepcopy(date_format)
        if heading == accouts_objects['heads'][-1]:
            date_style['text_wrap'] = True
            date_style['align'] = 'right'
            date_style['indent'] = 1
        worksheet.write(row, column, datetime.strptime(heading, '%Y-%m-%dT%H:%M:%S').strftime('%b %Y'), workbook.add_format(date_style))
    column = 0
    for excel_rows in accouts_objects['items']:
        row += 1
        row_style = copy.deepcopy(accouts_objects['data'][excel_rows]['style'])
        row_style['indent'] = 1
        if not excel_rows == 'blank':
            worksheet.set_row(row, 30)
        else:
            row_style['bottom'] = 0
        if not accouts_objects['data'][excel_rows]['values']:
            worksheet.merge_range(row, 0, row, len(accouts_objects['heads']), accouts_objects['data'][excel_rows]['name'], workbook.add_format(row_style))
        else:
            column = 0
            if accouts_objects['data'][excel_rows]['name'].upper() in highlight_row_heads:
                row_style['bg_color'] = '#556680'
                row_style['font_color'] = '#ffffff'
            worksheet.write(row, column, accouts_objects['data'][excel_rows]['name'], workbook.add_format(row_style))
            if len(accouts_objects['data'][excel_rows]['name'])>column1_width:
                column1_width = len(accouts_objects['data'][excel_rows]['name'])
            for excel_columns in accouts_objects['heads']:
                column += 1
                style_dict = copy.deepcopy(accouts_objects['data'][excel_rows]['style'])
                style_dict['num_format'] = '#,##0.00'
                if excel_columns == accouts_objects['heads'][-1]:
                    style_dict['align'] = 'right'
                    style_dict['indent'] = 1
                if accouts_objects['data'][excel_rows]['name'].upper() in highlight_row_heads:
                    style_dict['bg_color'] = '#556680'
                    style_dict['font_color'] = '#ffffff'
                if excel_rows.upper() in total_return_row_heads:
                    total_return_value += accouts_objects['data'][excel_rows]['values'].get(excel_columns, 0.00)
                worksheet.write(row, column, accouts_objects['data'][excel_rows]['values'].get(excel_columns, 0.00), workbook.add_format(style_dict))
    worksheet.set_column(0, 0, column1_width)
    worksheet.set_column(1, column-1, 12)
    worksheet.set_column(column, column, 15)
    worksheet.merge_range(row+1, 0, row+1, len(accouts_objects['heads'])+1, '', workbook.add_format({'right': 1, 'right_color': '#ffffff', 'bottom': 2}))
    worksheet.merge_range(0, len(accouts_objects['heads'])+1, row+1, len(accouts_objects['heads'])+1, '', workbook.add_format({'right': 2}))
    worksheet.set_row(row+1, 50)
    workbook.close()
    return total_return_value


def excel_row_data_formating(dct_data, accouts_objects, last_value, date, current_col_ind):
    if dct_data['items']:
        if f"Total {dct_data['name']}" not in accouts_objects['items']:
            if not accouts_objects['items'] or not last_value:
                accouts_objects['items'].append(f"Total {dct_data['name']}")
            else:
                accouts_objects['items'].insert(accouts_objects['items'].index(last_value), f"Total {dct_data['name']}")
            if current_col_ind in [0, 1]:
                accouts_objects['items'].insert(accouts_objects['items'].index(f"Total {dct_data['name']}")+1, 'blank')
        if f"Total {dct_data['name']}" not in accouts_objects['data']:
            accouts_objects['data'][f"Total {dct_data['name']}"] = {}
            accouts_objects['data'][f"Total {dct_data['name']}"]['name'] = f"Total {dct_data['name']}"
            accouts_objects['data'][f"Total {dct_data['name']}"]['style'] = copy.deepcopy(accouts_objects['settings']['default_styles'])
            accouts_objects['data'][f"Total {dct_data['name']}"]['style']['bold'] = True
            accouts_objects['data'][f"Total {dct_data['name']}"]['style']['bottom'] = 0
            accouts_objects['data'][f"Total {dct_data['name']}"]['column'] = current_col_ind
            accouts_objects['data'][f"Total {dct_data['name']}"]['data'] = {}
            accouts_objects['data'][f"Total {dct_data['name']}"]['data']['style'] = copy.deepcopy(accouts_objects['settings']['default_styles'])
            accouts_objects['data'][f"Total {dct_data['name']}"]['data']['style']['bold'] = True
            accouts_objects['data'][f"Total {dct_data['name']}"]['data']['style']['bottom'] = 0
            if current_col_ind == 0:
                accouts_objects['data'][f"Total {dct_data['name']}"]['style']['bg_color'] = '#e4ecf6'
                accouts_objects['data'][f"Total {dct_data['name']}"]['style']['font_color'] = '#1a1b23'
                accouts_objects['data'][f"Total {dct_data['name']}"]['data']['style']['bg_color'] = '#e4ecf6'
                accouts_objects['data'][f"Total {dct_data['name']}"]['data']['style']['font_color'] = '#1a1b23'
        accouts_objects['data'][f"Total {dct_data['name']}"]['data'][date] = {}
        accouts_objects['data'][f"Total {dct_data['name']}"]['data'][date]['value'] = dct_data['value']
        if dct_data['name'] not in accouts_objects['items']:
            accouts_objects['items'].insert(accouts_objects['items'].index(f"Total {dct_data['name']}"), dct_data['name'])
        if dct_data['name'] not in accouts_objects['data']:
            accouts_objects['data'][dct_data['name']] = {}
            accouts_objects['data'][dct_data['name']]['name'] = dct_data['name']
            accouts_objects['data'][dct_data['name']]['style'] = copy.deepcopy(accouts_objects['settings']['default_styles'])
            if current_col_ind == 0:
                accouts_objects['data'][dct_data['name']]['style']['font_color'] = '#1a1b23'
            accouts_objects['data'][dct_data['name']]['column'] = current_col_ind
            accouts_objects['data'][dct_data['name']]['data'] = {}
        current_col_ind += 1
        for data in dct_data['items']:
            excel_row_data_formating(data, accouts_objects, f"Total {dct_data['name']}", date, current_col_ind)
    else:
        if dct_data['name'] not in accouts_objects['items']:
            if not last_value:
                accouts_objects['items'].append(dct_data['name'])
            else:
                accouts_objects['items'].insert(accouts_objects['items'].index(last_value), dct_data['name'])
            accouts_objects['items'].insert(accouts_objects['items'].index(dct_data['name'])+1, f"Total {dct_data['name']}")
        if dct_data['name'] not in accouts_objects['data']:
            accouts_objects['data'][dct_data['name']] = {}
            accouts_objects['data'][dct_data['name']]['name'] = dct_data['name']
            accouts_objects['data'][dct_data['name']]['style'] = copy.deepcopy(accouts_objects['settings']['default_styles'])
            accouts_objects['data'][dct_data['name']]['column'] = current_col_ind
            if current_col_ind == 1:
                accouts_objects['data'][dct_data['name']]['style']['font_color'] = '#1a1b23'
                accouts_objects['data'][dct_data['name']]['style']['bold'] = True
            accouts_objects['data'][dct_data['name']]['data'] = {}
            accouts_objects['data'][dct_data['name']]['data']['style'] = copy.deepcopy(accouts_objects['settings']['default_styles'])
        if dct_data['value']:
            accouts_objects['data'][dct_data['name']]['data'][date] = {}
            accouts_objects['data'][dct_data['name']]['data'][date]['value'] = dct_data['value']

        if current_col_ind > accouts_objects['settings']['name_column_index']:
            accouts_objects['settings']['name_column_index'] = current_col_ind


def codat_balance_sheet_response(json_response, user_object, info):
    if not json_response or not json_response.get('reports'):
        return 0
    file_path = f'{settings.MEDIA_ROOT}/{settings.ON_BOARDING_DATA_BASE_PATH}/{str(user_object.id)}/' \
                f'{settings.ON_BOARDING_DATA_FILE_PATH}/'
    ...
    # with open(file_path + f"{info['type']}.json", 'w') as f:
    #     json.dump(json_response['reports'], f)
    default_styles = {'font_size': 8,
                    'font_color': '#6f749b',
                    'bg_color': '#ffffff',
                    'valign': 'vcenter',
                    'bottom': 1,
                    'border_color': '#e4ecf6'}
    accouts_objects = {'heads': [], 'items': [], 'data': {}, 'settings': {}}
    accouts_objects['settings']['name_column_index'] = -1
    accouts_objects['settings']['default_styles'] = default_styles
    accouts_objects['data']['blank'] = {}
    accouts_objects['data']['blank']['name'] = ''
    accouts_objects['data']['blank']['style'] = default_styles
    accouts_objects['data']['blank']['column'] = 0
    accouts_objects['data']['blank']['values'] = {}
    date_heads = ['date', 'fromDate', 'toDate']
    date_variable_name = ''
    for accounts in json_response['reports']:
        for date_key in date_heads:
            if accounts.get(date_key) and accounts[date_key] not in accouts_objects['heads']:
                accouts_objects['heads'].append(accounts[date_key])
                if not date_variable_name:
                    date_variable_name = date_key
                break
        for value in accounts:
            if value not in ['date']:
                if type(accounts[value]) == dict:
                    excel_row_data_formating(accounts[value], accouts_objects, '', accounts[date_variable_name], 0)
                elif type(accounts[value]) == float:
                    acc_name = re.sub(r"(\w)([A-Z])", r"\1 \2", value).title()
                    if acc_name not in accouts_objects['items']:
                        accouts_objects['items'].append(acc_name)
                        accouts_objects['items'].append('blank')
                    if acc_name not in accouts_objects['data']:
                        accouts_objects['data'][acc_name] = {}
                        accouts_objects['data'][acc_name]['name'] = acc_name
                        accouts_objects['data'][acc_name]['style'] = copy.deepcopy(accouts_objects['settings']['default_styles'])
                        accouts_objects['data'][acc_name]['column'] = 0
                        accouts_objects['data'][acc_name]['data'] = {}
                        accouts_objects['data'][acc_name]['data']['style'] = copy.deepcopy(accouts_objects['settings']['default_styles'])
                    accouts_objects['data'][acc_name]['data'][accounts[date_variable_name]] = {}
                    accouts_objects['data'][acc_name]['data'][accounts[date_variable_name]]['value'] = accounts[value]
    df = pd.DataFrame()
    writer = pd.ExcelWriter(file_path + f"{info['type']}.xlsx", engine='xlsxwriter')
    df.to_excel(writer, sheet_name=info['sub_title'])
    # Get the xlsxwriter workbook and worksheet objects.
    ...
    title_date['font_size'] = 11
    date_title = f"{datetime.strptime(accouts_objects['heads'][-1], '%Y-%m-%dT%H:%M:%S').strftime('%b %Y')} - {datetime.strptime(accouts_objects['heads'][0], '%Y-%m-%dT%H:%M:%S').strftime('%b %Y')}"
    worksheet.merge_range(0, 0, 0, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, info['sub_title'], workbook.add_format(title_head))
    worksheet.merge_range(1, 0, 1, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, info['title'].upper(), workbook.add_format(title_company))
    worksheet.merge_range(2, 0, 2, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, date_title, workbook.add_format(title_date))
    worksheet.merge_range(3, 0, 3, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, '', workbook.add_format(title_date))
    worksheet.set_row(0, 20)
    worksheet.set_row(1, 20)
    worksheet.set_row(2, 20)

    highlight_row_heads = ['NET PROFIT', 'NET ASSETS']
    total_return_row_heads = ['SALES', 'TOTAL LIABILITIES']
    total_return_value = 0
    row, column = 3, accouts_objects['settings']['name_column_index']
    if accouts_objects['settings']['name_column_index']<=1:
        row += 1
        date_style = copy.deepcopy(accouts_objects['settings']['default_styles'])
        date_style['font_color'] = '#556680'
        date_style['bg_color'] = '#F7FAFD'
        worksheet.write(row, 0, '', workbook.add_format(date_style))
        for date in accouts_objects['heads']:
            worksheet.write(row, column, datetime.strptime(date, '%Y-%m-%dT%H:%M:%S').strftime('%b %Y'), workbook.add_format(date_style))
            column += 1
    for excel_rows in accouts_objects['items']:
        if excel_rows not in accouts_objects['data']:
            continue
        row += 1
        if excel_rows == 'blank':
            row_style = copy.deepcopy(accouts_objects['data'][excel_rows]['style'])
            # row_style['right'] = 2
            # row_style['right_color'] = '#000000'
            if accouts_objects['settings']['name_column_index']>1:
                worksheet.merge_range(row, 0, row, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, '', workbook.add_format(row_style))
            else:
                worksheet.merge_range(row, 0, row, len(accouts_objects['heads'])+1, '', workbook.add_format(row_style))
        else:
            worksheet.set_row(row, 30)
            column = accouts_objects['settings']['name_column_index']-1
            if accouts_objects['settings']['name_column_index']>1 and accouts_objects['data'][excel_rows]['column'] == 0 and not accouts_objects['data'][excel_rows]['data']:
                date_style = copy.deepcopy(accouts_objects['settings']['default_styles'])
                date_style['font_color'] = '#556680'
                date_style['bg_color'] = '#F7FAFD'
                date_style['align'] = 'right'
                data_style = copy.deepcopy(accouts_objects['data'][excel_rows]['style'])
                data_style['bg_color'] = '#F7FAFD'
                data_style['font_size'] = 14
                data_style['bold'] = True
                data_style['text_wrap'] = True
                data_style['indent'] = 1
                worksheet.merge_range(row, 0, row, accouts_objects['settings']['name_column_index']-1, accouts_objects['data'][excel_rows]['name'], workbook.add_format(data_style))
                for date in accouts_objects['heads']:
                    column += 1
                    if date == accouts_objects['heads'][-1]:
                        date_style['text_wrap'] = True
                        date_style['indent'] = 1
                        # date_style['right'] = 2
                        # date_style['right_color'] = '#000000'
                    worksheet.write(row, column, datetime.strptime(date, '%Y-%m-%dT%H:%M:%S').strftime('%b %Y'), workbook.add_format(date_style))
            elif accouts_objects['settings']['name_column_index']>1 and accouts_objects['data'][excel_rows]['column'] == 1 and not accouts_objects['data'][excel_rows]['data']:
                data_style = copy.deepcopy(accouts_objects['data'][excel_rows]['style'])
                data_style['bold'] = True
                data_style['font_color'] = '#1a1b23'
                # data_style['right'] = 2
                # data_style['right_color'] = '#000000'
                data_style['text_wrap'] = True
                data_style['indent'] = 1
                worksheet.merge_range(row, 0, row, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, accouts_objects['data'][excel_rows]['name'], workbook.add_format(data_style))
                column = accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1
            else:
                col_start = accouts_objects['data'][excel_rows]['column']-1
                if col_start == -1:
                    col_start = 0
                name_style = copy.deepcopy(accouts_objects['data'][excel_rows]['style'])
                name_style['text_wrap'] = True
                name_style['indent'] = 1
                if accouts_objects['data'][excel_rows]['name'].upper() in highlight_row_heads:
                    name_style['bg_color'] = '#556680'
                    name_style['font_color'] = '#ffffff'
                    name_style['bold'] = True
                # if accouts_objects['settings']['name_column_index']>1 and accouts_objects['data'][excel_rows]['column'] == 1:
                #     name_style['font_color'] = '#1a1b23'
                #     name_style['bold'] = True
                if col_start-1>0:
                    worksheet.merge_range(row, 0, row, col_start-1, '', workbook.add_format(name_style))
                else:
                    worksheet.write(row, 0, '', workbook.add_format(name_style))
                if col_start == accouts_objects['settings']['name_column_index']-1:
                    worksheet.write(row, col_start, accouts_objects['data'][excel_rows]['name'], workbook.add_format(name_style))
                else:
                    worksheet.merge_range(row, col_start, row, accouts_objects['settings']['name_column_index']-1, accouts_objects['data'][excel_rows]['name'], workbook.add_format(name_style))
                if accouts_objects['data'][excel_rows]['data']:
                    for dates in accouts_objects['heads']:
                        column += 1
                        num_style = copy.deepcopy(accouts_objects['data'][excel_rows].get('style'))
                        if accouts_objects['data'][excel_rows]['name'].upper() in highlight_row_heads:
                            num_style['bg_color'] = '#556680'
                            num_style['font_color'] = '#ffffff'
                            num_style['bold'] = True
                        num_style['num_format'] = '#,##0.00'
                        if dates == accouts_objects['heads'][-1]:
                            # num_style['right'] = 2
                            # num_style['right_color'] = '#000000'
                            num_style['align'] = 'right'
                            num_style['indent'] = 1
                        if accouts_objects['data'][excel_rows]['name'].upper() in total_return_row_heads:
                            total_return_value += accouts_objects['data'][excel_rows]['data'].get(dates, {'value':0.0})['value']
                        worksheet.write(row, column, accouts_objects['data'][excel_rows]['data'].get(dates, {'value':0.0})['value'], workbook.add_format(num_style))
                else:
                    # name_style['right'] = 2
                    # name_style['right_color'] = '#000000'
                    worksheet.merge_range(row, accouts_objects['settings']['name_column_index'], row, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, '', workbook.add_format(name_style))
                    column = accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1

            # worksheet.write(row, column+1, '', workbook.add_format({'top': 1, 'bottom': 1, 'top_color': '#ffffff', 'bottom_color': '#ffffff'}))
    worksheet.set_column(0, accouts_objects['settings']['name_column_index']-2, 2)
    worksheet.set_column(accouts_objects['settings']['name_column_index']-1, accouts_objects['settings']['name_column_index']-1, 30)
    worksheet.set_column(accouts_objects['settings']['name_column_index'], accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-2, 12)
    worksheet.set_column(accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, 15)
    worksheet.merge_range(0, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads']), row, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads']), '', \
        workbook.add_format({'top': 1, 'bottom': 2, 'right': 2,'top_color': '#ffffff'}))
    worksheet.merge_range(row, 0, row, accouts_objects['settings']['name_column_index']+len(accouts_objects['heads'])-1, '', workbook.add_format({'right': 1, 'right_color': '#ffffff', 'bottom': 2}))
...
def codat_bank_statement_excel(json_response, user_object, info):
    ...
    df = pd.DataFrame()
    writer = pd.ExcelWriter(file_path + f"{info['type']}.xlsx", engine='xlsxwriter')
    df.to_excel(writer, sheet_name='Bank accounts')

    # Get the xlsxwriter workbook and worksheet objects.
    workbook = writer.book
    worksheet = writer.sheets['Bank accounts']
    # Add a header format.
    default_styles = {'text_wrap': True,
                    'font_size': 8,
                    'font_color': '#6f749b',
                    'bg_color': '#ffffff',
                    'valign': 'vcenter',
                    'bottom': 1,
                    'bottom_color': '#e4ecf6'}
    blank_styles = {'text_wrap': True,
                    'font_size': 8,
                    'font_color': '#6f749b',
                    # 'bg_color': '#ffffff',
                    'valign': 'vcenter'}
    head_style = copy.deepcopy(default_styles)
    head_style['font_color'] = '#556680'
    head_style['bg_color'] = '#F7FAFD'
    head_style['bold'] = True
    acc_head_style = copy.deepcopy(head_style)
    acc_head_style['indent'] = 1
    head_last = copy.deepcopy(head_style)
    # head_last['right'] = 2
    head_last['align'] = 'right'
    head_last['indent'] = 1
    num_style = copy.deepcopy(default_styles)
    num_style['num_format'] = '#,##0.00'
    # num_style['right'] = 2
    num_style['align'] = 'right'
    num_style['indent'] = 1
    account_style = copy.deepcopy(default_styles)
    account_style['indent'] = 1
    ...
    title_company = copy.deepcopy(title_head)
    title_company['font_size'] = 11
    worksheet.merge_range(0, 0, 0, 2, info['sub_title'], workbook.add_format(title_head))
    worksheet.merge_range(1, 0, 1, 2, info['title'].upper(), workbook.add_format(title_company))
    worksheet.merge_range(2, 0, 2, 2, '', workbook.add_format(title_company))
    worksheet.set_row(0, 20)
    worksheet.set_row(1, 20)
    worksheet.set_row(2, 20)
    worksheet.write(row, 0, 'Account', workbook.add_format(acc_head_style))
    worksheet.write(row, 1, 'Currency', workbook.add_format(head_style))
    worksheet.write(row, 2, 'Balance', workbook.add_format(head_last))
    worksheet.set_row(row, 30)
    for data in row_data:
        row += 1
        worksheet.set_row(row, 30)
        worksheet.write(row, 0, re.sub(' +', ' ', data.get('accountName', '')), workbook.add_format(account_style))
        worksheet.write(row, 1, data.get('currency', ''), workbook.add_format(default_styles))
        worksheet.write(row, 2, data.get('balance', '0.00'), workbook.add_format(num_style))
    row += 1
    worksheet.merge_range(0, 3, row, 3, '', workbook.add_format({'right': 2, 'bottom': 2}))
    worksheet.merge_range(row, 0, row, 2, '', workbook.add_format({'bottom': 2, 'right': 1, 'right_color': '#ffffff'}))
...
def password_set_send_email(subject, model_instance, recipient_email):
    """
    Function for sending email to the reset the password link
    ...
    # Imported inside function to prevent circular import error
    from .utility import send_email_utility
    password_set_link = f'{settings.FRONTEND_URL}{settings.FRONTEND_SIGN_UP_URL}{model_instance.slug_value}'
    message = render_to_string('registration/password_set.html', {
        'instance_data': model_instance,
