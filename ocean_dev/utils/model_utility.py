...
def on_board_remainder_email_send(subject, model_instance, recipient_email):
    """
    Function for sending email to remind complete email
    ...
    # Imported inside function to prevent circular import error
    from .utility import send_email_utility
    registration_link = f'{settings.FRONTEND_URL}{settings.FRONTEND_LOGIN_URL}'
    message = render_to_string('registration/on_board_reminder.html', {
        'instance_data': model_instance,
        'registration_link': registration_link,
        'logo_path': settings.BACKEND_URL[:-1] + settings.MEDIA_URL + 'logo/'
    })
