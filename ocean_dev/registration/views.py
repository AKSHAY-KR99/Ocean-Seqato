...
from utils.utility import user_activated_send_email, user_deactivated_send_email, generate_next_step_value, \
    ...
    codat_response, password_reset_send_email, codat_get_company_by_id, disconnect_codat, password_set_send_email
from utils.model_utility import on_board_remainder_email_send
    ...
    def retrieve(self, request, *args, **kwargs):
        if kwargs['pk'] == str(request.user.id) or request.user.user_role == settings.ADMIN["number_value"]:
            ...
            if hasattr(user_object.on_boarding_details, 'company_website'):
                ...
                output_data['company_website'] = False
            ...
            if sme_leads_object.exists():
                if output_data['company_website'] is False:
                    output_data['company_website'] = sme_leads_object[0].company_website
    ...
    def post(self, request):
        ...
        if login_tracker_object.exists():
            ...
            if user is not None:
                ...
                if user_object.is_on_board_remainder_send is not None:
                    if user_object.on_board_status == models.ON_BOARD_PASSWORD_SET and user_object.is_on_board_remainder_send is False:
                        lead_object = LeadsModel.objects.get(sign_up_email=user_object.email)
                        if lead_object.created_by is None:
                            on_board_remainder_email_send(subject=settings.ON_BOARD_REMAINDER,
                                                          model_instance=user_object,
                                                          recipient_email=user_object.email)
                            user_object.is_on_board_remainder_send = True
                            user_object.save()
                else:
                    user_object.is_on_board_remainder_send = False
                    user_object.save()
...
class ActivatingUserView(APIView):
    ...
    def post(self, request):
        ...
        if request.data['activate_action_status']:
            ...
            user_object.save()
            if user_object.user_role == settings.SUPPLIER["number_value"]:
                lead_supplier = get_object_or_404(LeadsModel, sign_up_email=request.data['email'], is_deleted=False)
                if lead_supplier.created_by is not None:
                    notification_data = {
                        "notification": "Supplier was activated by admin",
                        "type": settings.SME_SUPPLIER_ACTIVATED,
                        "description": f"Your supplier {lead_supplier.company_name} was activated",
                        "assignee": lead_supplier.created_by.id
                    }
                    supplier_notification_serializer = NotificationModelSerializer(data=notification_data)
                    if supplier_notification_serializer.is_valid(raise_exception=True):
                        supplier_notification_serializer.save()
            ...
            lead_object.current_status = ON_BOARDING_CUSTOMER
            lead_object.save()
            return Response({'message': 'User account has been rejected.'}, status=status.HTTP_200_OK)
...
class LoggedInUserDetail(APIView):
    ...
    def get(self, request):
        ...
        if request.user.user_role not in [settings.ADMIN["number_value"], settings.FACTOR["number_value"]]:
...
class UserPasswordResetView(APIView):
    ...
    def post(self, request):
        ...
        user_object = get_object_or_404(models.User, email=request.data['email'], is_deleted=False)
        if user_object.on_board_status == models.ON_BOARD_REJECTED:
            return Response({'message': 'Your account has been rejected'}, status=status.HTTP_400_BAD_REQUEST)
        elif user_object.on_board_status != models.ON_BOARD_USER_CREATED:
            ...
            password_set_send_email(subject=settings.PASSWORD_RESET, model_instance=user_object,
                                    recipient_email=user_object.email)
            user_object.is_reset_password = True
            user_object.save()
            return Response({"message": "Successfully sent the password reset link to the user"},
...
class CodatVisualizeView(APIView):
    ...
    def post(self, request):
        ...
        if 'id' in codat_connection_respose[0]:
            redirect_uri = settings.CODAT_URI + lead_object.company_id + '/dataConnection/' + codat_connection_respose[
                0].get("id")
