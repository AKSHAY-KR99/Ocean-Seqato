...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    is_on_board_remainder_send = models.BooleanField(null=True, blank=True)
...
def send_email_user_created_signal(instance, **kwargs):
    ...
    if instance.on_board_status == ON_BOARD_USER_CREATED and instance.is_deleted is False and not instance.is_mail_send:
        lead_instance = LeadsModel.objects.filter(sign_up_email=instance.email)
        if lead_instance.exists():
            lead_object = lead_instance[0]
            if lead_object.created_by is not None:
                if lead_object.created_by.user_role == settings.ADMIN['number_value'] or lead_object.created_by.user_role == \
                        settings.SME['number_value']:
                    user_created_send_email(subject=settings.USER_CREATED, model_instance=instance,
