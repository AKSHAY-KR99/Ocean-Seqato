...
class UserModelSerializers(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if lead_object.exists():
            ...
            response_data['company_type'] = lead_object[0].company_type
        if response_data['on_boarding_details']:
            response_data['company_name'] = ''
            if instance.on_boarding_details.company_name:
                response_data['company_name'] = instance.on_boarding_details.company_name.capitalize()
            ...
            if lead_object.exists():
                response_data['company_name'] = ''
                if lead_object[0].company_name:
                    response_data['company_name'] = lead_object[0].company_name.capitalize()
                response_data['website'] = lead_object[0].company_website
...
class UserLoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        user_object = User.objects.filter(email=data.get("email"))
        if user_object and not user_object[0].is_active:
