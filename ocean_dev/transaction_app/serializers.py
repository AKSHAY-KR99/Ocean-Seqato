...
class NotificationModelSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        response_data = super().to_representation(instance)
        if 'shipment_fund_invoice' in response_data:
            response_data['shipment_fund_invoice'] = int(response_data['shipment_fund_invoice'])
        if response_data['type'] == settings.SHIPMENT_ADDED_PAYMENT_PENDING:
            response_data['payment_type'] = models.PAYMENT_TO_SUPPLIER_BY_ADMIN
        fund_invoice_instance = models.FundInvoiceModel.objects.filter(pk=response_data['fund_invoice'])
        if fund_invoice_instance.exists():
            response_data['invoice_number'] = fund_invoice_instance[0].invoice_number
