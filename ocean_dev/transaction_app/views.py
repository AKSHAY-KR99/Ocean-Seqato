    ...
    def create(self, request, *args, **kwargs):
        if request.user.is_user_onboard and request.user.user_role == settings.SME['number_value']:
            ...
            if 'invoice_files' not in request.FILES:
                return Response({
                    "detail": "For creating a new fund invoice request, you will need an invoice file"},
            ...
            if float(request.POST["invoice_total_amount"]) <= available_amount:
                ...
                if country_data and country_data[0].get('shipping_date'):
                ...
                if 'other_document' in request.FILES:
                    for file_object in request.FILES.getlist('other_document'):
                        invoice_files_list.append(models.FundInvoiceFilesModel(fund_invoice=fund_invoice_data,
                                                                           file_object=file_object))
        ...
        if request.user.user_role == settings.SUPPLIER["number_value"] or \
            ...
            if not payment_object.exists():
                payment_notification = {
                    ...
                    "assignee": None,
                    "fund_invoice": fund_invoice_object.id
                }
            ...
            if len(country_model_ids)>1:
                shipment_mode = models.TRANSPORT_MODE_MIXED
            if shipment_mode:
                fund_invoice_object.transport_mode = shipment_mode
                fund_invoice_object.save()
        ...
        if request.user.user_role == settings.SME["number_value"]:
            ...
            if shipment_object.fund_invoice.fund_invoice_status.first().action_taken == \
                ...
                if kwargs['user_action'] == settings.SHIPMENT_ACKNOWLEDGED:
                    ...
                    lead_sme = contact_app_models.LeadsModel.objects.get(sign_up_email=shipment_object.fund_invoice.sme.email)
                    notification_data = {"shipment": shipment_object.id, "notification": "Shipment Send back by SME",
                                         ...
                                         "description": f"The shipment was returned by {lead_sme.company_name}",
                                         "assignee": shipment_object.fund_invoice.supplier.id,
                                         "fund_invoice": shipment_object.fund_invoice.id}
            ...
            if shipment_object.fund_invoice.fund_invoice_status.first().action_taken == \
                ...
                if kwargs['user_action'] == settings.SHIPMENT_ACKNOWLEDGED:
                    ...
                    shipment_send_back_email(subject=settings.EMAIL_SHIPMENT_SEND_BACK,
                                             ...
                                             sender_name=request.user.first_name)
                    lead_supplier = contact_app_models.LeadsModel.objects.get(sign_up_email=shipment_object.fund_invoice.supplier.email)
                    notification_data = {"shipment": shipment_object.id,
                                         ...
                                         "description": f"The shipment was returned by {lead_supplier.company_name}",
...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        status_serializer_data.is_valid(raise_exception=True)
        status_serializer_data.save()
        if "payment_ref_number" in request.data and request.data['payment_ref_number'] is not None \
                and request.data['payment_ref_number'] != "null":
            payment_reference_number = request.POST['payment_ref_number']
        else:
            payment_reference_number = False
        if int(request.POST['payment_type']) == models.PAYMENT_TO_SUPPLIER_BY_ADMIN:
            payment_acknowledgment_mail(settings.PAYMENT_ACKNOWLEDGE_SUPPLIER,
                                        fund_invoice_object.supplier.on_boarding_details.company_name,
                                        settings.SENDER_NAME, fund_invoice_object.supplier.email,
                                        request.POST['paying_amount'], payment_reference_number)
            payment_acknowledgment_mail_sme(settings.PAYMENT_ACKNOWLEDGE_SUPPLIER,
                                            fund_invoice_object.sme.on_boarding_details.company_name,
                                            request.POST['paying_amount'],
                                            fund_invoice_object.supplier.on_boarding_details.company_name,
        ...
        if sign_doc_data['status']:
            if request.user.user_role == settings.ADMIN['number_value']:
                ...
                notification_data = {"contract": contract_object.id,
                                     ...
                                     "description": "SME signatures are pending",
                                     "assignee": assignee
                                     }
                notification_serializer = serializers.NotificationModelSerializer(data=notification_data)
                if notification_serializer.is_valid(raise_exception=True):
            ...
            if request.user.user_role == settings.SME['number_value']:
                ...
                if contract_object.is_master_contract:
                    ...
                    notification_data = {
                        ...
                        "description": "Shipment Upload is Pending",
                        "assignee": fund_invoice_object.sme.id,
                        "fund_invoice": contract_object.fund_invoice.id
                    }
                    notification_serializer = serializers.NotificationModelSerializer(data=notification_data)
                    if notification_serializer.is_valid(raise_exception=True):
                        notification_serializer.save()
...
class DeleteFundInvoice(APIView):
    ...
    def post(self, request, **kwarg):
        ...
        if notification_obj.exists():
            for notifications in notification_obj:
...
class GetContractNumber(APIView):
    ...
    def get(self, request):
        try:
            contract_object = models.ContractModel.objects.filter(contract_number__startswith="0-20").order_by('id').last()
        except:
