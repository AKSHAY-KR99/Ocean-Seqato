...
from . import models
from registration.models import User, UserDetailModel, LoginTrackerModel, ON_BOARD_PASSWORD_SET, OTP_SENT_STRING
from registration.serializers import UserModelSerializers
from registration.validators import validate_password
...
from utils.utility import create_user, contact_info_send_email, codat_company_creation, delete_codat_company, \
    send_otp_for_login_or_set_password
    ...
    def create(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            ...
            if request.data["role"] not in [settings.SME["number_value"], settings.SUPPLIER["number_value"]]:
                return Response({'detail': 'Please check the role value entered'},
                                status=status.HTTP_400_BAD_REQUEST)
            if "company_type" not in request.data:
                return Response({'detail': 'Please enter company type'},
                                status=status.HTTP_400_BAD_REQUEST)
            if "password" not in request.data:
                return Response({'detail': 'Please enter password'},
                                status=status.HTTP_400_BAD_REQUEST)
            try:
            ...
            serializer_data = self.serializer_class(data=input_dict)
            if serializer_data.is_valid():
                lead_user = serializer_data.save()
            else:
                if "sign_up_email" in serializer_data.errors:
                    return Response({'detail': serializer_data.errors['sign_up_email'][0]},
                                    status=status.HTTP_400_BAD_REQUEST)
                    ...
                    return Response(serializer_data.errors, status=status.HTTP_400_BAD_REQUEST)
            create_user(lead_user)
            user_instance = User.objects.filter(email=input_dict['sign_up_email'], is_deleted=False)
            if not user_instance.exists():
                return Response({'detail': 'An error occurred while creating a user'},
                                status=status.HTTP_400_BAD_REQUEST)
            user_object = user_instance[0]
            user_object.is_active = True
            user_object.on_board_status = ON_BOARD_PASSWORD_SET
            user_object.set_password(request.data['password'])
            user_object.save()

            otp_value = int(random.randint(100000, 999999))
            login_tracker_object = LoginTrackerModel.objects.create(user=user_object, otp_value=otp_value,
            ...
            return Response({
                ...
                'message': 'User created and password set, OTP sent to the user phone number',
...
class EmailExistenceCheck(APIView):
    """
    Class for check entered email is existing or not
    """
    permission_classes = [AllowAny]

    def post(self, request):
        if 'company_email' not in request.data or request.data['company_email'] is None or request.data[
            'company_email'] == "":
            return Response({"detail": "Please provide email address"}, status=status.HTTP_400_BAD_REQUEST)
        email = request.data['company_email']
        lead_object = models.LeadsModel.objects.filter(sign_up_email=email)
        user_object = User.objects.filter(email=email)
        if lead_object.exists():
            return Response({'detail': 'Email entered already exist',
                             'emailExist': True}, status=status.HTTP_400_BAD_REQUEST)
        elif user_object.exists():
            return Response({'detail': 'Email entered already exist',
                             'emailExist': True}, status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({'emailExist': False}, status=status.HTTP_200_OK)


class PhoneNumberExistenceCheck(APIView):
    """
    Class for check entered phone number is existing or not
    """
    permission_classes = [AllowAny]

    def post(self, request):
        if 'phone_number' not in request.data or request.data['phone_number'] is None or request.data[
            'phone_number'] == "":
            return Response({"detail": "Please provide Phone number"}, status=status.HTTP_400_BAD_REQUEST)
        phone_number = request.data['phone_number']
        lead_object = models.LeadsModel.objects.filter(sign_up_phone_number=phone_number)
        user_object = User.objects.filter(phone_number=phone_number)
        if lead_object.exists():
            return Response({'detail': 'Phone number entered already exist',
                             'phoneNumberExist': True}, status=status.HTTP_400_BAD_REQUEST)
        elif user_object.exists():
            return Response({'detail': 'Phone number entered already exist',
                             'phoneNumberExist': True}, status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({'phoneNumberExist': False}, status=status.HTTP_200_OK)
