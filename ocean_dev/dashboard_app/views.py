...
class DashboardSmeInfo(APIView):
    ...
    def get(self, request):
        if request.user.user_role == settings.SME["number_value"]:
            ...
            invoice_amount = FundInvoiceModel.objects.filter(sme=user_object.id, is_deleted=False).aggregate(Sum('total_sales_amount'))
            ...
            repayment_amount = (invoice_amount['total_sales_amount__sum'] if invoice_amount['total_sales_amount__sum'] else 0) - \
                (sme_paid['paying_amount__sum'] if sme_paid['paying_amount__sum'] else 0)
            ...
            output_dict = {
                "data": {"cards": {"total_credit_limit": total_credit_limit, "available": available_credit,
                                   ...
                                   "total_invoice_number": FundInvoiceModel.objects.filter(sme=user_object.id,
                                    ...
                                    "supplier_count": User.objects.filter(user_role=3, is_user_onboard=True,
